...
from jwql.utils.logging_functions import (
    configure_logging,
    log_fail,
    log_info,
    make_log_file,
...
def test_make_log_file():
    ...
    correct_locations = [
        os.path.join(
            get_config()["log_dir"], "dev", module, os.path.basename(log_file)
        ),
        os.path.join(
            get_config()["log_dir"], "test", module, os.path.basename(log_file)
        ),
        os.path.join(
            get_config()["log_dir"], "prod", module, os.path.basename(log_file)
        ),
    ]
