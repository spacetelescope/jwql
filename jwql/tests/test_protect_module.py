...
---
    ...
    !!!WARNING IF TESTING LOCALLY!!!
    These tests will call protect_module.py::lock_module.py and will sent emails to jwql.stsci.edu!
    Please locally change the `message['To']` recipient to an appropriate address while testing to
    avoid spamming/confusing the team.
...
@mark.skipif(ON_GITHUB_ACTIONS, reason='Requires access to central storage.')
def test_already_locked_module_wont_run_with_bad_lock_file(module_lock):
...
@mark.skipif(ON_GITHUB_ACTIONS, reason='Requires access to central storage.')
def test_already_locked_module_wont_run_with_legit_lock_file(module_lock):
...
@mark.skipif(ON_GITHUB_ACTIONS, reason='Requires access to central storage.')
def test_lock_module_handles_stale_lock_files(module_lock):
