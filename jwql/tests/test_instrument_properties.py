...
def test_amplifier_info():
    ...
    fullframe = instrument_properties.amplifier_info(
        os.path.join(data_dir, "test_image_ff.fits")
    )
    fullframe_truth = (
        4,
        {
    ...
    fullframe = instrument_properties.amplifier_info(
        os.path.join(data_dir, "test_image_ff.fits"), omit_reference_pixels=False
    )
    fullframe_truth = (
        4,
        {
    ...
    subarray = instrument_properties.amplifier_info(
        os.path.join(data_dir, "test_image_1.fits")
    )
    subarray_truth = (1, {"1": [(0, 0), (10, 10)]})
    assert subarray == subarray_truth

    subarray_one = instrument_properties.amplifier_info(
        os.path.join(data_dir, "test_image_grismstripe_one_amp.fits")
    )
    subarray_one_truth = (1, {"1": [(4, 4), (2044, 64)]})
    assert subarray_one == subarray_one_truth

    subarray_four = instrument_properties.amplifier_info(
        os.path.join(data_dir, "test_image_grismstripe_four_amp.fits")
    )
    subarray_four_truth = (
        4,
        {
...
def test_calc_frame_time():
    ...
    nircam_160 = 0.27864
    nrc_fullframe = instrument_properties.calc_frame_time(
        "nircam", "NRCA1_FULL", 2048, 2048, 4
    )
    nrc_160 = instrument_properties.calc_frame_time(
        "nircam", "NRCA1_SUB160", 160, 160, 1
    )
    nrs_fullframe = instrument_properties.calc_frame_time(
        "niriss", "NIS_CEN", 2048, 2048, 4
    )
    # nrs_some_subarra = instrument_properies.calc_frame_time('niriss', '????', ??, ??, ?)

    print(
        "STILL NEED TO ADD FRAMETIME CALCS FOR MIRI AND NIRSPEC TO THE CALC_FRAME_TIME_FUNCTION"
    )
