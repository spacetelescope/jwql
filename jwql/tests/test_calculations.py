...
def test_double_gaussian_fit():
    ...
    params, sigma = calculations.double_gaussian_fit(
        bin_centers, input_values, initial_params
    )
...
def test_gaussian1d_fit():
    ...
    assert np.isclose(peak[0], mean_value, atol=0.0015, rtol=0.0)
    assert np.isclose(width[0], sigma_value, atol=0.0015, rtol=0.0)
    assert (mean_value <= peak[0] + 3 * peak[1]) & (mean_value >= peak[0] - 3 * peak[1])
    assert (sigma_value <= width[0] + 3 * width[1]) & (
        sigma_value >= width[0] - 3 * width[1]
    )
