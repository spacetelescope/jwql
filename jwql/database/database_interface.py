...
if (
    "build"
    and "project" in socket.gethostname()
    or os.path.expanduser("~") == "/home/jenkins"
):
...
class FilesystemInstrument(base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "date", "instrument", "filetype", name="filesystem_instrument_uc"
        ),
    )
    ...
    instrument = Column(
        Enum(*JWST_INSTRUMENT_NAMES, name="instrument_enum"), nullable=False
    )
    ...
    def colnames(self):
        ...
        # Get the columns
        a_list = [
            col for col, val in self.__dict__.items() if not isinstance(val, datetime)
        ]
...
def get_monitor_columns(data_dict, table_name):
    ...
    for column_definition in column_definitions:
        ...
        if dtype in list(data_type_dict.keys()):
            if array:
                data_dict[column_name.lower()] = Column(
                    ARRAY(data_type_dict[dtype], dimensions=dimension)
                )
            else:
...
def monitor_orm_factory(class_name):
    ...
    data_dict["__table_args__"] = (
        UniqueConstraint(
            "id", "entry_date", name="{}_uc".format(data_dict["__tablename__"])
        ),
    )
