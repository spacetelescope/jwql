...
def power_ice(conn, start, end):
    ...
    sql_c = (
        "SELECT * FROM SE_ZIMIRICEA_IDLE WHERE start_time BETWEEN "
        + start_str
        + " AND "
        + end_str
        + " ORDER BY start_time"
    ...
    sql_c = (
        "SELECT * FROM SE_ZIMIRICEA_HV_ON WHERE start_time BETWEEN "
        + start_str
        + " AND "
        + end_str
        + " ORDER BY start_time"
    ...
    scat1 = p.scatter(
        x="start_time", y="average", color="orange", legend="Power idle", source=idle
    )
    scat2 = p.scatter(
        x="start_time", y="average", color="red", legend="Power hv on", source=hv
    )
    p.line(
        x="start_time", y="average", color="orange", legend="Power idle", source=idle
    )
    ...
    for index, item in _hv.iterrows():
        ...
        err_ys_hv.append(
            (item["average"] - item["deviation"], item["average"] + item["deviation"])
        )
...
def power_fpea(conn, start, end):
    ...
    sql_c = (
        "SELECT * FROM SE_ZIMIRFPEA WHERE start_time BETWEEN "
        + start_str
        + " AND "
        + end_str
        + " ORDER BY start_time"
    ...
    _fpea["start_time"] = pd.to_datetime(
        Time(_fpea["start_time"], format="mjd").datetime
    )
    ...
    # add a line renderer with legend and line thickness
    scat1 = p.scatter(
        x="start_time", y="average", color="orange", legend="Power FPEA", source=fpea
    )
    p.line(
        x="start_time", y="average", color="orange", legend="Power FPEA", source=fpea
    )
...
def currents(conn, start, end):
    ...
    a = pf.add_to_plot(
        p, "ICE Current idle", "SE_ZIMIRICEA_IDLE", start, end, conn, color="red"
    )
    b = pf.add_to_plot(
        p, "ICE Current HV on", "SE_ZIMIRICEA_HV_ON", start, end, conn, color="orange"
    )
    c = pf.add_to_plot(
        p, "FPE Current", "SE_ZIMIRFPEA", start, end, conn, color="brown"
    )
