...
def vdetcom(conn, start, end):
    ...
    a = pf.add_to_plot(
        p, "VDETCOM IC", "IGDP_MIR_IC_V_VDETCOM", start, end, conn, color="red"
    )
    b = pf.add_to_plot(
        p, "VDETCOM SW", "IGDP_MIR_SW_V_VDETCOM", start, end, conn, color="orange"
    )
    c = pf.add_to_plot(
        p, "VDETCOM LW", "IGDP_MIR_LW_V_VDETCOM", start, end, conn, color="green"
    )
...
def vssout(conn, start, end):
    ...
    a = pf.add_to_plot(
        p, "VSSOUT IC", "IGDP_MIR_IC_V_VSSOUT", start, end, conn, color="red"
    )
    b = pf.add_to_plot(
        p, "VSSOUT SW", "IGDP_MIR_SW_V_VSSOUT", start, end, conn, color="orange"
    )
    c = pf.add_to_plot(
        p, "VSSOUT LW", "IGDP_MIR_LW_V_VSSOUT", start, end, conn, color="green"
    )
...
def vrstoff(conn, start, end):
    ...
    a = pf.add_to_plot(
        p, "VRSTOFF IC", "IGDP_MIR_IC_V_VRSTOFF", start, end, conn, color="red"
    )
    b = pf.add_to_plot(
        p, "VRSTOFF SW", "IGDP_MIR_SW_V_VRSTOFF", start, end, conn, color="orange"
    )
    c = pf.add_to_plot(
        p, "VRSTOFF LW", "IGDP_MIR_LW_V_VRSTOFF", start, end, conn, color="green"
    )
...
def vdduc(conn, start, end):
    ...
    a = pf.add_to_plot(
        p, "VDDUC IC", "IGDP_MIR_IC_V_VDDUC", start, end, conn, color="red"
    )
    b = pf.add_to_plot(
        p, "VDDUC SW", "IGDP_MIR_SW_V_VDDUC", start, end, conn, color="orange"
    )
    c = pf.add_to_plot(
        p, "VDDUC LW", "IGDP_MIR_LW_V_VDDUC", start, end, conn, color="green"
    )
...
def bias_plots(conn, start, end):
    ...
    l = gridplot([[plot2, plot1], [plot3, plot4], [plot5, None]], merge_tools=False)
