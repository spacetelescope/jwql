...
def extract_filterpos(condition, nominals, ratio_mnem, pos_mnem):
    ...
    for index, pos in enumerate(pos_mnem):
        ...
        if pos['value'] != pos_bevore:
            pos_new = []
            pos_new['name'] = pos['value']
            pos_new['start_t'] = pos['time']
            pos_new['end_t'] = pos['time']
            pos_ranges.append(pos_new)


        else:
            pos_ranges[-1]['end_t'] = pos['time']
...
def once_a_day_routine(mnemonic_data):
    ...
    for identifier in mn.mnemonic_cond_1:
        ...
        if data != None:
            ...
            print_string = DateTime.now().isoformat() + ' Process 15min: ' + identifier + '\t' + 'cond1 Success'
            print(print_string)
        else:
            # print("no data for {}".format(identifier))
            print_string = DateTime.now().isoformat() + ' Process 15min: ' + identifier + '\t' + 'cond1 no data'
    ...
    for identifier in mn.mnemonic_cond_2:
        ...
        if data != None:
            returndata.update({identifier: data})
            print_string = DateTime.now().isoformat() + ' Process 15min: ' + identifier + '\t' + 'cond2 Success'
            print(print_string)
        else:
            print_string = DateTime.now().isoformat() + ' Process 15min: ' + identifier + '\t' + 'cond2 no data'
            print('\33[31m' + print_string + '\033[0m')
