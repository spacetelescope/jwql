...
from jwql.instrument_monitors.miri_monitors.data_trending.utils.process_data import (
    whole_day_routine,
    wheelpos_routine,
)
...
directory = os.path.join(
    get_config()["outputs"], "miri_data_trending", "trainings_data_day", "*.CSV"
)
...
def process_file(conn, path):
    ...
    for pos in mn.gw_positions:
        try:
            ...
            for element in data_GW14:
                sql.add_wheel_data(
                    conn, "IMIR_HK_GW14_POS_RATIO_{}".format(pos), element
                )
            for element in data_GW23:
                sql.add_wheel_data(
                    conn, "IMIR_HK_GW23_POS_RATIO_{}".format(pos), element
                )
        except KeyError:
            pass
    ...
    for pos in mn.ccc_positions:
        try:
            ...
            for element in data:
                sql.add_wheel_data(
                    conn, "IMIR_HK_CCC_POS_RATIO_{}".format(pos), element
                )
        except KeyError:
            pass
