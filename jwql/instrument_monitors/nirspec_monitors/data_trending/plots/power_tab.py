...
def ice_power(conn, start, end):
    ...
    e = pf.add_to_plot(
        p, "VMOTOR", "INRSH_HK_VMOTOR", start, end, conn, color="burlywood"
    )
    ...
    h = pf.add_to_plot(
        p, "ADCTGAIN", "INRSH_HK_ADCTGAIN", start, end, conn, color="brown"
    )
    i = pf.add_to_plot(
        p, "ADCOFFSET", "INRSH_HK_ADCTOFFSET", start, end, conn, color="navy"
    )
    p.add_layout(
        LinearAxis(
...
def mce_power(conn, start, end):
    ...
    b = pf.add_to_plot(
        p, "MCE A current", "SE_ZINRSMCEA", start, end, conn, color="blue"
    )
...
def fpe_power(conn, start, end):
    ...
    d = pf.add_to_plot(p, "N15V", "INRSH_HK_N15V", start, end, conn, color="orange")
    e = pf.add_to_plot(
        p, "N12C", "INRSD_ALG_ACC_N12C", start, end, conn, color="burlywood"
    )
    f = pf.add_to_plot(
        p, "1D5", "INRSD_ALG_ACC_3D3_1D5_C", start, end, conn, color="green"
    )
    g = pf.add_to_plot(
        p, "Chassis", "INRSD_ALG_CHASSIS", start, end, conn, color="purple"
    )
    p.add_layout(
        LinearAxis(
            y_range_name="current",
