...
from jwql.database.database_interface import (
    NIRCamDarkQueryHistory,
    NIRCamDarkPixelStats,
    NIRCamDarkDarkCurrent,
)
from jwql.database.database_interface import (
    NIRISSDarkQueryHistory,
    NIRISSDarkPixelStats,
    NIRISSDarkDarkCurrent,
)
from jwql.database.database_interface import (
    MIRIDarkQueryHistory,
    MIRIDarkPixelStats,
    MIRIDarkDarkCurrent,
)
from jwql.database.database_interface import (
    NIRSpecDarkQueryHistory,
    NIRSpecDarkPixelStats,
    NIRSpecDarkDarkCurrent,
)
from jwql.database.database_interface import (
    FGSDarkQueryHistory,
    FGSDarkPixelStats,
...
from jwql.utils.utils import (
    ...
    get_config,
    filesystem_path,
    initialize_instrument_monitor,
    update_monitor_table,
)

THRESHOLDS_FILE = os.path.join(
    os.path.split(__file__)[0], "dark_monitor_file_thresholds.txt"
)
...
def mast_query_darks(instrument, aperture, start_date, end_date):
    ...
    for template_name in dark_template:
        ...
        parameters = {
            "date_obs_mjd": {"min": start_date, "max": end_date},
            "apername": aperture,
            "exp_type": template_name,
        }

        query = monitor_mast.instrument_inventory(
            instrument,
...
class Dark:
        ...
        if not testing:
            ...
            for instrument in ["nircam"]:
                ...
                for aperture in possible_apertures:
                    ...
                    logging.info(
                        "\tQuery times: {} {}".format(self.query_start, self.query_end)
                    )

                    # Query MAST using the aperture and the time of the
                    # most recent previous search as the starting time
                    new_entries = mast_query_darks(
                        instrument, aperture, self.query_start, self.query_end
                    )
                    logging.info(
                        "\tAperture: {}, new entries: {}".format(
                    ...
                    if len(new_entries) >= file_count_threshold:
                        ...
                        logging.info(
                            "\tSufficient new dark files found for {}, {} to run the dark monitor.".format(
                        ...
                        self.data_dir = os.path.join(
                            self.output_dir,
                            "data/{}_{}".format(
                                self.instrument.lower(), self.aperture.lower()
                            ),
                        )
                        ...
                        logging.info(
                            (
                                ...
                                "required to run dark current monitor."
                            ).format(
                                len(new_entries),
                                instrument,
                                aperture,
    ...
    def add_bad_pix(
        self, coordinates, pixel_type, files, mean_filename, baseline_filename
    ):
    ...
    def exclude_existing_badpix(self, badpix, pixel_type):
        ...
        db_entries = (
            session.query(self.pixel_table)
    ...
    def find_hot_dead_pixels(
        self, mean_image, comparison_image, hot_threshold=2.0, dead_threshold=0.1
    ):
        ...
        zeros = comparison_image == 0.0
        comparison_image[zeros] = 1.0
        mean_image[zeros] += 1.0
    ...
    def get_baseline_filename(self):
        ...
        subq = (
            ...
            .group_by(self.pixel_table.detector)
            .subquery("t2")
        )
    ...
    def most_recent_search(self):
        ...
        sub_query = (
            ...
            .group_by(self.query_table.aperture)
            .subquery("t2")
        )
        ...
        query = (
            session.query(self.query_table)
            .join(
                sub_query,
                and_(
    ...
    def noise_check(self, new_noise_image, baseline_noise_image, threshold=1.5):
        ...
        # Avoid divide by zeros
        zeros = baseline_noise_image == 0.0
        baseline_noise_image[zeros] = 1.0
        new_noise_image[zeros] += 1.0
    ...
    def run(self, file_list):
        ...
        for filename in file_list:
            ...
            if any(steps_to_run.values()) is False:
                ...
                if not os.path.isfile(processed_file):
                    ...
                    logging.info(
                        "\tSlope file {} already exists. Skipping call to pipeline.".format(
                            processed_file
                        )
                    )
                    pass
        ...
        slope_image, stdev_image = calculations.mean_image(
            slope_image_stack, sigma_threshold=3
        )
        mean_slope_file = self.save_mean_slope_image(
            slope_image, stdev_image, slope_files
        )
        logging.info(
        ...
        if aperture_type == "FULLSCA":
            ...
            self.add_bad_pix(
                new_hot_pix, "hot", file_list, mean_slope_file, baseline_file
            )
            self.add_bad_pix(
                new_dead_pix, "dead", file_list, mean_slope_file, baseline_file
            )
            ...
            self.add_bad_pix(
                new_noisy_pixels, "noisy", file_list, mean_slope_file, baseline_file
            )
    ...
    def save_mean_slope_image(self, slope_img, stdev_img, files):
        ...
        mean_slope_dir = os.path.join(
            get_config()["outputs"], "dark_monitor", "mean_slope_images"
        )
    ...
    def stats_by_amp(self, image, amps):
        ...
        for key in amps:
            ...
            amp_mean, amp_stdev = calculations.mean_stdev(
            ...
            gauss_fit_model = models.Gaussian1D(
                amplitude=amplitude[0], mean=peak[0], stddev=width[0]
            )
            gauss_fit = gauss_fit_model(bin_centers)

            positive = hist > 0
            degrees_of_freedom = len(hist) - 3.0
            total_pix = np.sum(hist[positive])
            p_i = gauss_fit[positive] / total_pix
            gaussian_chi_squared[key] = (
                np.sum((hist[positive] - (total_pix * p_i) ** 2) / (total_pix * p_i))
                / degrees_of_freedom
            )
            ...
            if key == "5":
                if self.instrument.upper() in ["NIRISS", "NIRCAM"]:
                    ...
                    double_gaussian_params[key] = [
                        [param, sig]
                        for param, sig in zip(double_gauss_params, double_gauss_sigma)
                    ]
                    double_gauss_fit = calculations.double_gaussian(
                        bin_centers, *double_gauss_params
                    )
                    degrees_of_freedom = len(bin_centers) - 6.0
                    dp_i = double_gauss_fit[positive] / total_pix
                    double_gaussian_chi_squared[key] = (
                        np.sum(
                    ...
                    double_gaussian_params[key] = [[0.0, 0.0] for i in range(6)]
                    double_gaussian_chi_squared[key] = 0.0
            else:
                double_gaussian_params[key] = [[0.0, 0.0] for i in range(6)]
                double_gaussian_chi_squared[key] = 0.0
        ...
        logging.info(
            "\tStandard deviation of dark rate by amplifier: {}".format(amp_means)
        )
        logging.info(
        ...
        logging.info(
            "\tReduced chi-squared associated with double Gaussian fit: {}".format(
                double_gaussian_chi_squared
            )
        )
