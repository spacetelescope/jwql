...
---
    ...
    >>>con_set = [ cond.equal(m.get_mnemonic_table('IMIR_HK_POM_LOOP'),'OFF'),
...
class equal(condition):
    ...
    def cond_true_time(self):
        ...
        for key in self.mnemonic_table:
...
class greater(condition):
    ...
    def __init__(self, mnemonic, value):
        ...
        ----------
        mnemonic : astropy table
            includes mnemonic engineering data and corresponding primary time
        value : str
    ...
    def cond_true_time(self):
        """Filters all values that are greater than a given comparison value
            if greater: Primary time -> temp_start
            if not greater: Primary time -> temp_end
        ...
        for mnemonic_tab in self.mnemonic_table:
            #find all times whose Raw values are grater than the given value
            if float(mnemonic_tab['value']) > self.value:
                temp_start.append(mnemonic_tab["time"])

            #find all end values
            else:
                temp_end.append(mnemonic_tab["time"])
