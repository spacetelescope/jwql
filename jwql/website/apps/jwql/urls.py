...
urlpatterns = [
    ...
    re_path(r'^ajax/(?P<inst>({}))/(?P<file_root>.+)_(?P<filetype>.+)/explore_image/scaling_(?P<scaling>.+)/low_(?P<low_lim>.+)/high_(?P<high_lim>.+)/ext_(?P<ext_name>.+)/$'.format(instruments), views.explore_image_ajax, name='explore_image_ajax'),
