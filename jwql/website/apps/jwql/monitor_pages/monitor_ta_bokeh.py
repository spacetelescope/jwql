...
def collect_visit_data(output_dir):
    ...
    for vd in visit_dirs:
        vid = vd.parts[-1]
        visit = {'path': vd}
        lsr_file = vd / (vid + '_least_squares_dict_replicaTA.json')
        with lsr_file.open() as f:
            visit['replica_ls'] = json.load(f)
        lso_file = vd / (vid + '_oss_least_squares_output_replicaTA.json')
        with lso_file.open() as f:
...
class MonitorTA(BokehTemplate):
    ...
    def pre_init(self, **kwargs):
        ...
        self.output_dir = Path(self.settings['outputs']) / 'monitor_ta'
