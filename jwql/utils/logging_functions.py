...
def log_info(func):
    ...
    def wrapped(*args, **kwargs):
        ...
        logging.info(
            "Elapsed Real Time: {}:{}:{}".format(
                int(hours_time), int(minutes_time), int(seconds_time)
            )
        )
        logging.info(
            "Elapsed CPU Time: {}:{}:{}".format(
                int(hours_cpu), int(minutes_cpu), int(seconds_cpu)
            )
        )
...
def log_timing(func):
    ...
    def wrapped(*args, **kwargs):
        ...
        minutes_time, seconds_time = divmod(remainder_time, 60)
        logging.info(
            "Elapsed Real Time of {}: {}:{}:{}".format(
                func.__name__, int(hours_time), int(minutes_time), int(seconds_time)
            )
        )
        logging.info(
            "Elapsed CPU Time of {}: {}:{}:{}".format(
                func.__name__, int(hours_cpu), int(minutes_cpu), int(seconds_cpu)
            )
        )
